    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for ending_idx, (context, ending) in enumerate(
            ...
            inputs = tokenizer.encode_plus(
                ...
                truncation_strategy="only_second",  # 常にcontextをtruncate
            )
...
def evaluate(args, model, tokenizer, prefix="", test=False):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        acc = simple_accuracy(pred_ids, out_label_ids)
        ...
        with open(output_eval_file, "w") as fp:
            for pred_id, out_label_id in zip(pred_ids, out_label_ids):
                fp.write("{} {}\n".format(pred_id, out_label_id))
