...
def convert_example_to_features(example,cache_dir):
    ...
    for ending in example.endings:
        ...
        #それ以外の場合には、足りない部分を0で埋める。
            ...
            padding_length=MAX_SEQ_LENGTH-(text_features_length+image_features_length)
            zero_padding=torch.zeros(padding_length).cuda().float()
            input_ids=torch.cat([input_ids,zero_padding],dim=0)

            for i in range(text_features_length+image_features_length,MAX_SEQ_LENGTH):
                attention_mask[i]=0
    ...
    for epoch in range(EPOCH_NUM):
        ...
        for step,batch in enumerate(tqdm(train_dataloader)):
            ...
            if step%log_interval==0:
                logger.info("進捗: {:.2f} %\t損失: {}".format(step*len(batch)/len(train_dataloader),loss.item()))
...
if __name__=="__main__":
    ...
    train_dataset=create_input_features_dataset(TRAIN_JSON_FILENAME,TRAIN_FEATURES_DIR)
